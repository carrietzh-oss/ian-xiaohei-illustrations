# Modo Scenes (花野稀泥 2.0) — condensado Grok
...
```text
花野稀泥 + objeto real + ação física + rótulos curtos + vazio narrativo
```
...
- 花野稀泥 **executa** a ação (mesmo critério de `huayexini-ip.md`: se tirar ele e a cena “funciona sozinha”, falhou).  
...
- 5–8 nós: cada um com objeto real + 花野稀泥 + 1–3 linhas de anotação.  
...
At each node: real prop + 花野稀泥 doing a small physical action + 1-3 short handwritten notes in {pt-BR|Chinese}.
Start left: {início}. End right: {conclusão}. Facts only from user brief: {fatos}.
Clean, airy, photographic props, deadpan-cute 花野稀泥 with the selected backpack or head-worn variant, no corporate infographic, no logos unless user provided.
```
